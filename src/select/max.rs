...
impl<I> Selector<I> for MaximizeSelector
    ...
    fn select(&self, population: &[I]) -> Result<Parents<I>, String> {
        ...
        let trunc: Vec<I> = scored.into_iter()
            .take(self.count)
            .map(|(_,ind)| ind)
            .collect();
...
mod tests {
    ...
    fn test_count_zero() {
        ...
        let population: Vec<Test> = (0..100).map(|i| Test { f: i as f64 }).collect();
    ...
    fn test_result_ok() {
        ...
        assert!(selector.select(&population).unwrap()[0].0.calculate_fitness() == 99.0);
    }
